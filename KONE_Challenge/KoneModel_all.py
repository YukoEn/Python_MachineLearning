...
def main():
    ...
    df = pd.read_csv("dataBefore_all.csv")
    ...
    plt.figure(5)
    plt.subplot(2, 1, 1)
    plt.plot(sampleId, lm.predict(X_train), 'go', markersize=2, alpha=0.5, label='Predicted call time [s] vs Id \nLinear Regression')
    plt.legend(loc="upper right", bbox_to_anchor=[1, 1],
    ncol=2, shadow=True, fancybox=True)
    plt.xlim((0, 10000))
    plt.ylim((0, 50))
    ...
    plt.plot(np.linspace(1,96,96), Y_train[0:96], '-bo', linewidth=0.5, markersize=2, alpha=0.5, label='input train data before change')   
    plt.legend(loc="upper right", bbox_to_anchor=[1, 1],
    ncol=2, shadow=True, fancybox=True)
    plt.xlim((0, 100))
    plt.ylim((0, 50))
    ...
    plt.plot(np.linspace(1,9100,9100), Y_train[0:9100], '-mo',linewidth=0.5, markersize=2, alpha=0.5, label='input train data')
    plt.legend(loc="upper right", bbox_to_anchor=[1, 1],
    ncol=2, shadow=True, fancybox=True)
    plt.xlim((0, 9100))
    plt.ylim((0, 50))
